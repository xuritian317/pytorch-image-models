...
class Butterfly200():
    def __init__(self, root, is_train=True, data_len=None, transform=None, level='species'):
        self.root = root
        self.is_train = is_train
        self.transform = transform
        self.level = level
        ...
        img_txt = 'Butterfly200_train_release.txt' if self.is_train else 'Butterfly200_test_release.txt'

        # with open(os.path.join(self.root, img_txt)) as f:
        #     for txt in f.readlines():

        txt = open(os.path.join(self.root, img_txt))
        for line in txt:
            image_name, species_label, genus_label, subfamily_label, family_label = line.strip().split(' ')
            img_name_list.append(image_name)
            species.append(int(species_label))
            genus.append(int(genus_label))
            subfamily.append(int(subfamily_label))
            family.append(int(family_label))

        self.images = [imageio.imread(os.path.join(self.root, 'images_small', img_name)) for img_name in
    ...
    def __getitem__(self, index):
        img, target = self.images[index], self.labels[index] - 1,

        if len(img.shape) == 2:
            img = np.stack([img] * 3, 2)
        img = Image.fromarray(img, mode='RGB')

        species_label = self.labels[index] - 1
        genus_label = self.genus[index] - 1
        subfamily_label = self.subfamily[index] - 1
        family_label = self.family[index] - 1
